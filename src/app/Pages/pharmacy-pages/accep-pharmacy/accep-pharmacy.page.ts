...
@Component({
  selector: 'app-accep-pharmacy',
  templateUrl: './accep-pharmacy.page.html',
  styleUrls: ['./accep-pharmacy.page.scss'],
})
export class AccepPharmacyPage implements OnInit {
  pharmacyData: any;
  page: number;
  perPage = 0;
  totalData: number;
  ...
  requestData() {
    this.pharmacyServ.getPharmcyHistoryAccept()
        .subscribe(res => {
              this.result = res;
  ...
  loadData(event) {
    ...
    setTimeout(() => {
      this.pharmacyServ.getPharmcyHistoryAccept(this.page)
          .subscribe(
              res => {
                this.result = res;
                this.perPage = this.result.per_page;
